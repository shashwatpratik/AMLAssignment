...
class net3D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel: tuple = (5, 5, 5), act_layer=nn.ReLU):
        super().__init__()
        self.conv1 = nn.Conv3d(in_channels, out_channels, kernel)
        self.bn1 = nn.BatchNorm3d(out_channels)
        self.act1 = act_layer()
        self.conv2 = nn.Conv3d(out_channels, out_channels, kernel)
...
def resnet50_fc512(in_channel, num_classes, pretrained=True, **kwargs):
    model = ResNet(
...
def CNN3D(in_channels, out_channels):
    model = net3D(in_channels, out_channels)
    return model